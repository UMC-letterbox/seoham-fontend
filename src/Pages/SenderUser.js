...
const SenderUser = () => {
  ...
  */
  useEffect(() => {
    fetch(`https://seohamserver.shop/posts/senders/${sender}`, {
      method: "GET",
      headers: {
  ...
  const deleteTag = () => {
    ...
    /* 보낸이 id 값 확인 필요 메시지... */
    fetch(`https://seohamserver.shop/posts/senders/delete/${sender}`, {
      method: "DELETE",
      headers: {
        "x-access-token": localStorage.getItem("login_token"),
      },
    ...
    return (
      <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M88.9344 110.246C72.1782 110.246 58.3578 122.816 56.3882 139.04C56.1219 141.233 57.9384 143.033 60.1475 143.033H139.852C142.062 143.033 143.878 141.233 143.612 139.04C141.642 122.816 127.822 110.246 111.066 110.246H88.9344Z"
          fill="#FFF2F2"
        />
        <circle
          cx="100"
  ...
  return (
    ...
    >
      ...
      {isopen ? (
        <Menubox
          menuContents={["선택", "보낸이 정보 수정", "보낸이 정보 삭제"]}
          menuFunc={[selectTag, modiTag, deleteTag]}
        />
      ) : null}
      {modal === true ? (
        <Modal_sender modalClose={setModal} sender={sender} />
      ) : null}
    </div>
  );
};
