...
const ModifyLetter = () => {
  ...
  function getPostInfo(){
    ...
    .then((res) => {
      ...
      console.log(rawDate);
      setDaySelected(
        [rawDate.substr(0,4), rawDate.substr(5,2), rawDate.substr(8,2)]
      );
      let ttemp = [...p.tagIdx];
      let tttemp = ttemp.map(t => t.toString());
      setTestTag([...tttemp]);
      setPaperSelected(p.letterIdx);
      setData({
        userIdx: userId,
        sender: p.sender,
        content: p.content,
        date: p.date,
        tagIdx : p.tagIdx,
        letterIdx: p.letterIdx,
        postidx: p.postIdx,
      });
    })
  }
  console.log('편지지 번호', paperSelected);
  ...
  return (
    <div className={`${theme} overflow-scroll`}>
      ...
      <div className="flex flex-col justify-center items-center">
        <textarea
          className="w-72 h-72 rounded-t drop-shadow-lg text-black dark:bg-[#303435]"
